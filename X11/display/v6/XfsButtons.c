...
void DataTransmitted(int type, int preset_nb, int op_nb, int wave_type, int mode_type, int level_set, int amp_value, int time_value, int mod_coef, int arr_choice) {
	if (type == TYPE_PRESET) {
		...
		*(uint32_t *)input_arbitration_addr = preset_bit << 31;
	}
	if (type == TYPE_USER_SETTING) {
		...
		*(uint32_t *)input_arbitration_addr = setting_mode_bit << 32 | op_bit << 31 | op_wave_bit << 28 | op_mode_bit << 26 | level_set_bit << 25 | amp_value_bit << 22 | time_value_bit << 15 | mod_coef_bit << 8 | arr_choice_bit << 6;
